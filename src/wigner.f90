module wigner 
    ...
    use iso_fortran_env, only: dpres=>real64
...
!------------------------------------------------------------------------------
    ...
    real(dpres), intent(out) :: w3j(:)
    integer(int32), intent(out), optional :: exitstatus
    real(dpres) :: wnmid, wpmid, scalef, denom, rs(j2+j3+1), &
    ...
    if (jnum == 1) then
        w3j(1) = 1.0_dpres / sqrt(2.0_dpres * jmin + 1.0_dpres)
        
        if ( (w3j(1) < 0.0_dpres .and. (-1)**(j2-j3+m2+m3) > 0) .or. &
            (w3j(1) > 0.0_dpres .and. (-1)**(j2-j3+m2+m3) < 0) ) &
            w3j(1) = -w3j(1)
    ...
    if (m1 == 0 .and. m2 == 0 .and. m3 == 0) then       ! All m's are zero
        wl(jindex(jmin)) = 1.0_dpres
        wl(jindex(jmin+1)) = 0.0_dpres
        jn = jmin + 1

    else if (yjmin == 0.0_dpres) then           ! The second terms is either zero
        if (xjmin == 0.0_dpres) then            ! or undefined
            flag1 = 1
            jn = jmin
    ...
    else if (xjmin * yjmin >= 0.0_dpres) then
        ...
        do j = jmin + 1, jmax-1, 1
            ...
            if (abs(xj) > abs(denom) .or. xj * denom >= 0.0_dpres .or. &
                    denom == 0.0_dpres) then
                jn = j - 1
                exit
    ...
    if (m1 == 0 .and. m2 == 0 .and. m3 == 0) then
        wu(jindex(jmax)) = 1.0_dpres
        wu(jindex(jmax-1)) = 0.0_dpres
        jp = jmax - 1

    else if (yjmax == 0.0_dpres) then
        if (zjmax == 0.0_dpres) then
            flag2 = 1
            jp = jmax
    ...
    else if (yjmax * zjmax >= 0.0_dpres) then
        wu(jindex(jmax)) = 1.0_dpres
        wu(jindex(jmax-1)) = - yjmax / zjmax
        jp = jmax - 1
        ...
        do j=jmax-1, jn, -1
            ...
            if (abs(zj) > abs(denom) .or. zj * denom >= 0.0_dpres .or. &
                    denom == 0.0_dpres) then
                jp = j + 1
                exit
    ...
    if (flag1 == 0) then
        ...
        do j = jn, jmid - 1, 1
            ...
            if (abs(wl(jindex(j+1))) > 1.0_dpres) then
                ! watch out for overflows.
                wl(jindex(jmin):jindex(j+1)) = wl(jindex(jmin):jindex(j+1)) &
                                               / scalef
            end if

            if (abs(wl(jindex(j+1)) / wl(jindex(j-1))) < 1.0_dpres .and. &
                wl(jindex(j+1)) /= 0.0_dpres) then
        ...
        do j = jp, jmid + 1, -1
            wu(jindex(j-1)) = - (x(j)*wu(jindex(j+1)) + y(j)*wu(jindex(j)) ) &
                                / z(j)
            if (abs(wu(jindex(j-1))) > 1.0_dpres) then
        ...
        ! rescale two sequences to common midpresoint
        if (jmid == jmax) then
            w3j(1:jnum) = wl(1:jnum)
    ...
    CONTAINS
        ...
        subroutine fixsign
            ...
            if ( (w3j(jindex(jmax)) < 0.0_dpres .and. (-1)**(j2-j3+m2+m3) > 0) &
                    .or. (w3j(jindex(jmax)) > 0.0_dpres .and. &
