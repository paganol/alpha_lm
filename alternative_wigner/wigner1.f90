...
!C
      integer :: NDIM, IER
      real(dp) :: L2, L3, M2, M3, THRCOF(NDIM)
      integer :: L1MIN, L1MAX
!C
      integer :: I, INDEX, LSTEP, N, NFIN, NFINP1, NFINP2, NFINP3, NLIM, NSTEP2
      real(dp) :: A1, A1S, A2, A2S, C1, C1OLD, C2, CNORM, D1MACH
      real(dp) :: DENOM, DV, EPS, HUGE, L1, M1, NEWFAC, OLDFAC
      real(dp) :: ONE, RATIO, SIGN1, SIGN2, SRHUGE, SRTINY, SUM1
      real(dp) :: SUM2, SUMBAC, SUMFOR, SUMUNI, THREE, THRESH
      real(dp) :: TINY, TWO, X, X1, X2, X3, Y, Y1, Y2, Y3, ZERO
!C
...
!C  Check error conditions 1 and 2.
      IF((L2-ABS(M2)+EPS.LT.ZERO).OR.(L3-ABS(M3)+EPS.LT.ZERO))THEN
         IER=1
         write(*,*)'L2-ABS(M2) or L3-ABS(M3) less than zero'
         RETURN
      ELSEIF((MOD(L2+ABS(M2)+EPS,ONE).GE.EPS+EPS).OR.(MOD(L3+ABS(M3)+EPS,ONE).GE.EPS+EPS))THEN
         IER=2
         write(*,*)'L2+ABS(M2) or L3+ABS(M3) not integer'
         RETURN
      ENDIF
!C
!C
...
!C  Check error condition 4.
      IER=4
      write(*,*)'L1MIN greater than L1MAX'
      RETURN
!C
...
!C  Check error condition 5.
   21 IER = 5
      write(*,*)'Dimension of result array for 3j coefficients too small.'
      RETURN
!C
!C
